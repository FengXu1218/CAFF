...
res_order_cyclebase3.0 <- build_pseudo_order(cyclebase3.0_cc_cpm_new, 
                                             reverse = T)
# THETA1 <- res_order_cyclebase3.0$THETA
# THETA2 <- res_order_cyclebase3.0$THETA
...
inter_sample <- Reduce(intersect, list(colnames(cc_gene_test_cpm_new),
                                       colnames(Seurat_cc_cpm_new),
                                       colnames(GO0007049_cc_cpm_new),
                                       colnames(cyclebase3.0_cc_cpm_new)))

Theta_data <- data.frame(
  THETA_of_Seurat = res_order_Seurat$THETA[inter_sample],
  THETA_of_cc_gene_test = res_order_cc_gene_test$THETA[inter_sample],
  THETA_of_GO0007049 = res_order_GO0007049$THETA[inter_sample],
  THETA_of_cyclebase3.0 = res_order_cyclebase3.0$THETA[inter_sample]
)
...
write.csv(Theta_data, "./Hela1/X_FFTF_data/THETA_data.csv")


Theta_data1 <- Theta_data

# Theta_data1 <- Theta_data+pi
# 
# for (i in 1:4) {
#   for (j in 1:nrow(Theta_data1)) {
#     if (!(Theta_data1[j,i] < Theta_data1[1,i])) {
#       Theta_data1[j,i] <- Theta_data1[j,i] - Theta_data1[1,i]
#     } else {
#       Theta_data1[j,i] <- Theta_data1[j,i] - Theta_data1[1,i] + 2*pi
#     }
#   }
# }
# Theta_data1 <- Theta_data1 -pi
# 调用cor_scatter_plot绘制最大化相关性散点图：
source("functions.R")
#p_list2 <- circ_cor_scatter_plot2(Theta_data1,cor_method = "circ_cor")
#p_list2 <- circ_cor_scatter_plot2(Theta_data1,cor_method = "Spearman_cor")
p_list2 <- circ_cor_scatter_plot3(Theta_data1,cor_method = "Spearman_cor")

data_list <- p_list2[[2]]

library(cowplot)

p2 <- plot_grid(plotlist = p_list2[[1]], ncol = 3)
p2

ggsave("./Hela1/plots/cor_of_THETA3.pdf", plot = p2, height = 8, width = 12)
